...
- **⏭ Open (need an interactive editor session — full-screen passes / gameplay-coupled tuning):** Phase 2 volumetric fog/god-rays renderer feature; Phase 3 SSR renderer feature + water refraction; Phase 4 migrating the combat/warp/weather cube-FX to `ParticleSystem` + VFX Graph + URP decals.

**Build verify:** all of the above compiles + builds a 325 MB player via `scripts/build-client.ps1 -SkipPrereqs` (Unity 6000.4.9f1). NOT yet visually tuned (needs a human eye in-game) and NOT committed.
