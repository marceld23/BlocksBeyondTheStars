...
- **VFX migration off the cube-FX ✅ (started):** ParticleSystem **engine-flame thruster** (24c0f9c, verified), **explosion** fireball+sparks (fe910a8), **mining/place debris puff** + reusable `ParticleAlpha` shader (97e76b5, verified). `Particle`(additive)+`ParticleAlpha`(alpha) shaders are the toolkit for the rest.
- **⏭ Open:** weather VFX (rain/snow/ash → particles, tie to fog weather), weapon/warp VFX, URP decals; Phase 3 **SSR** renderer feature + water refraction; the **volumetric god-rays** as an AFTER-post pass (the darkening-safe redesign).
