...
A small HUD button (bottom-right) and the **F1** hotkey open a modal where any player sends a bug report or a feature wish (one form: title, description, optional e-mail, privacy hint). On Send it grabs a full-frame screenshot **with the HUD but without the dialog** (captured at open, before the screen dims) and fires **both** paths: a client-direct HTTPS POST to `www.blocksbeyondthestars.com/_functions/bugreport`, and the existing `/bump` message so the server still writes its rich local snapshot on own/SP servers.
...
- **Unity**: [FeedbackUi.cs](client/Assets/BlocksBeyondTheStars/Scripts/FeedbackUi.cs) (button + dialog + capture + dual send), wired in [WorldRig.cs](client/Assets/BlocksBeyondTheStars/Scripts/WorldRig.cs). New icon [btn_feedback.png](client/Assets/Resources/icons/btn_feedback.png) (OpenAI, speech-bubble+pencil).
...
- **Open**: stand up the Wix/Velo endpoint per the requirements doc, then add the `WIX_BUGREPORT_API_KEY` Environment secret + the CI step; confirm the website privacy note matches `ui.feedback.hint`. **NEEDS Unity build to verify the dialog/button + that `System.Net.Http` resolves in the player (first client HttpClient use).**
