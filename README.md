...
**You don't need Unity, a game engine licence, or a powerful machine to help here.** The Unity Editor
builds the playable client — but the server, the shared code, world generation, all game content and the
entire test suite are plain C# and JSON, and everything our CI checks runs on the free .NET 10 SDK alone.
...
```bash
git clone https://github.com/marceld23/BlocksBeyondTheStars.git
cd BlocksBeyondTheStars
dotnet build BlocksBeyondTheStars.sln
dotnet test --filter "Category!=Slow"
```

That is the same thing our CI runs. Full details in **[CONTRIBUTING.md](CONTRIBUTING.md)** and, for a
deeper tour of the architecture and conventions, **[AGENTS.md](AGENTS.md)**.

**Before you open a pull request, three things worth knowing:**

- **CI is free, fast and works from forks.** Four required checks (`dotnet format`, `ruff`, `actionlint`,
  tests), roughly five minutes, no Unity licence and no repository secrets involved.
- **One click of paperwork.** The first time you open a PR, our CLA bot asks you to sign the
  [Contributor License Agreement](docs/legal/CLA.md) with your GitHub account. It exists so the game can
  one day also reach consoles; the open version stays AGPL and free forever. The reasoning is written out
  in [CONTRIBUTING.md](CONTRIBUTING.md#licensing--the-contributor-license-agreement-cla) — worth reading
  *before* you start, so nothing is a surprise afterwards.
- **Every merged contributor gets credited** — here in this README *and* in the game's own Credits screen,
  in all 14 languages. That's a real name in a released game, not a line in a file nobody opens.

🤖 **Working with an AI coding agent — or are one?** You're welcome here, openly: this whole game was
built that way. [AGENTS.md](AGENTS.md) is written for exactly that, and agent-authored pull requests are
credited by name like any other. One thing to settle first: the CLA has to be signed by the **human who
owns the GitHub account**, because it is a statement about copyright that only a person can make.
...
([how that works](CONTRIBUTING.md#credits--how-your-name-gets-into-the-game)).
