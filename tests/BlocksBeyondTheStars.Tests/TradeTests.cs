...
public sealed class TradeTests : IDisposable
{
    ...
    public TradeTests()
    {
        _root = Path.Combine(Path.GetTempPath(), "bbts_trade_" + Guid.NewGuid().ToString("N"));
    ...
    private static (BlocksBeyondTheStars.Shared.State.PlayerState a, BlocksBeyondTheStars.Shared.State.PlayerState b) TwoTradersTogether(SvGameServer server)
    {
