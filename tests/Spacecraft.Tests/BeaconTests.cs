...
public sealed class BeaconTests : IDisposable
{
    ...
    public BeaconTests()
    {
        _root = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "spacecraft_beacon_" + Guid.NewGuid().ToString("N"));
    ...
    private static Spacecraft.GameServer.PlayerSession Builder(SvGameServer server, string name = "Builder")
    {
