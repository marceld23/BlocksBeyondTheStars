...
public sealed class AdminContentTests : IDisposable
{
    ...
    {
        _install = Path.Combine(Path.GetTempPath(), "spacecraft_admincontent_" + Guid.NewGuid().ToString("N"));
    ...
    private static MissionDefinition ValidMission(string id) => new()
    {
        ...
        Rewards = { new Spacecraft.Shared.Definitions.ItemAmount("iron_plate", 2) },
    };
