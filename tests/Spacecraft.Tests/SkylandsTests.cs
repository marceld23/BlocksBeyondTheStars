            ...
            foreach (int cy in new[] { 4, 5, 6 })
            {
                ...
                for (int lz = 0; lz < WorldConstants.ChunkSize && !foundIsland; lz++)
                {
                    ...
                    for (int ly = 0; ly < WorldConstants.ChunkSize; ly++)
                    {
                        ...
                        var below = gen.Generate(planet, WorldConstants.WorldToChunk(new Spacecraft.Shared.Geometry.Vector3i(wx, surface + 2, wz)));
                        var bo = WorldConstants.ChunkOrigin(WorldConstants.WorldToChunk(new Spacecraft.Shared.Geometry.Vector3i(wx, surface + 2, wz)));
