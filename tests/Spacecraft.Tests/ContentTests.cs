    ...
    public void Validation_DetectsBrokenReference()
    {
        var ex = Assert.Throws<ContentValidationException>(() =>
        {
            var content = new GameContent(
                blocks: new[]
                {
                    new Spacecraft.Shared.Definitions.BlockDefinition
                    {
                        Key = "stone",
                        Drops = { new Spacecraft.Shared.Definitions.ItemAmount("nonexistent_item", 1) },
                    },
                },
                items: Array.Empty<Spacecraft.Shared.Definitions.ItemDefinition>(),
                recipes: Array.Empty<Spacecraft.Shared.Definitions.RecipeDefinition>(),
                blueprints: Array.Empty<Spacecraft.Shared.Definitions.BlueprintDefinition>(),
                shipModules: Array.Empty<Spacecraft.Shared.Definitions.ShipModuleDefinition>(),
