...
public sealed class WorldOptionsTests : IDisposable
{
    ...
    {
        _root = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "spacecraft_wopt_" + Guid.NewGuid().ToString("N"));
    ...
    public void FloraFactorZero_GeneratesABarrenSurface()
    {
        ...
        bool HasFlora(WorldGenerator gen)
        {
            ...
            {
                var chunk = gen.Generate(planet, new Spacecraft.Shared.World.ChunkCoord(cx, cy, cz));
                for (int i = 0; i < chunk.RawBlocks.Length; i++)
                {
                    var key = _content.BlockById(new Spacecraft.Shared.Primitives.BlockId(chunk.RawBlocks[i]))?.Key;
