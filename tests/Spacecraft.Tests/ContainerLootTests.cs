...
public sealed class ContainerLootTests : IDisposable
{
    ...
    {
        _root = Path.Combine(Path.GetTempPath(), "spacecraft_loot_" + Guid.NewGuid().ToString("N"));
    ...
    private static void DieAt(SvGameServer server, Spacecraft.Shared.State.PlayerState p, Vector3f where)
    {
