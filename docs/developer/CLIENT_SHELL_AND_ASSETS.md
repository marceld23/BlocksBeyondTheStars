...
- Audio (UI, ship hums, tools, doors/airlocks/docking, medbay/respawn, alarms, combat):
  ...
  shell). **Superseded:** sound effects, ambience and the four short synth-mood music loops are now
  bundled (`client/Assets/Resources/audio/*.mp3`, plus procedural/synth audio) and play in-game. The
  **Suno track library** (40 full-length MP3s, 164 MB) is deliberately *not* a `Resources/` asset: it
  lives in `client/Music/` (tracked, outside `Assets/`), is synced to the git-ignored
  `client/Assets/StreamingAssets/music/` by `scripts/sync-client-libs.{ps1,sh}` and is **streamed on
  demand** by `ClientMusic` (`UnityWebRequestMultimedia`) — so the WebGL player data file no longer
  carries the whole library (#1167; see `MUSIC_TRACKS.md` and `client/Music/README.md`).
...
client/Assets/BlocksBeyondTheStars/
  ...
  Audio/ui/  Audio/ship/  Audio/tools/  Audio/ambient/  (later)
client/Assets/Resources/audio/ # bundled SFX/ambience + the 4 synth-mood music loops (exists)
client/Assets/StreamingAssets/ # git-ignored, generated by scripts/sync-client-libs.{ps1,sh}:
  data/                       # data-driven definitions + locales (source: repo-root data/)
  music/                      # Suno track library, streamed on demand (source: client/Music/, #1167)
  server/                     # bundled singleplayer server (scripts/publish-local-server; desktop only)
```

Rule of thumb (#1167): anything under `client/Assets/Resources/` is baked into the WebGL `.data` file every
browser visitor downloads up front — keep large or rarely-needed binaries out of `Assets/` and stream them
from StreamingAssets instead (never under `StreamingAssets/data/`, whose manifest the browser prefetches).
